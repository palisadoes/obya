...
def env():
    ...
    if 'OBYA_CONFIGDIR' not in os.environ:
        log_message = (
            'The OBYA_CONFIGDIR environment variable not set.')
        log2die_safe(1039, log_message)

    # Make sure the OBYA_CONFIGDIR environment variable is set to unittest
    if 'unittest' in os.environ['OBYA_CONFIGDIR'].lower():
        log_message = ('''\
The OBYA_CONFIGDIR is set to a unittest directory. Daemon cannot be run''')
